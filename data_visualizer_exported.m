classdef data_visualizer_exported < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        disance_button          matlab.ui.control.ToggleButton
    ...
    methods (Access = private)
        ...
        function run_visualizer(app, event)
            ...
            for i = 1:NUM_VARS
                ...
                curr = axes(i);
                hold(ax, "on")
                curr(1) = plot(ax, time, selected_data(i,:), "Color", [1,0,0]);
                %curr(2) = plot(ax, time, data2(i,:), "Color", [0,0.5,1]);
                hold(ax, "off")
                %% 
            ...
            % ax.XTick = time(1):10:time(end);
            ax.XTick = 0:25:time(end);
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            % Create disance_button
            app.disance_button = uitogglebutton(app.plot_wrt_group);
            app.disance_button.Text = 'distance';
            app.disance_button.Position = [11 10 100 23];
