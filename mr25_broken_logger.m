classdef mr25_broken_logger < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function run_visualizer(app, event)
            ...
            lap_min = data{1, "Lap_Number"};
            lap_max = data{end, "Lap_Number"};
            ...
            axis_checkboxes = [app.time_check, app.brake_bias_check, app.distance_check, ...
            ...
            if lapA == false
                if app.wrt_time
                    lapA_x = data{:,"timestamp"};
                else
                    lapA_x = data{:,"Distance_Traveled"};
                end
            ...
            % Else, do it to specific laps(s)
                ...
                lapA_data{:,"timestamp"} = lapA_data{:,"timestamp"} - lapA_data{1,"timestamp"};
                if lapB
                    lapB_data{:,"timestamp"} = lapB_data{:,"timestamp"} - lapB_data{1,"timestamp"};
                end

                lapA_data{:,"Distance_Traveled"} = lapA_data{:,"Distance_Traveled"} - lapA_data{1,"Distance_Traveled"};
                if lapB
                    lapB_data{:,"Distance_Traveled"} = lapB_data{:,"Distance_Traveled"} - lapB_data{1,"Distance_Traveled"};
                end

                % New variable just for domain
                if app.wrt_time
                    lapA_x = lapA_data{:,"timestamp"};
                    if lapB
                        lapB_x = lapB_data{:,"timestamp"};
                    end
                else
                    lapA_x = lapA_data{:,"Distance_Traveled"};
                    if lapB
                        lapB_x = lapB_data{:,"Distance_Traveled"};
                    end
                end
            ...
            lat_g = data_checkbox{:, "X_Accel"};
            long_g = data_checkbox{:, "Y_Accel"};

            scatter(ax, lat_g, long_g, 20, "blue", "Marker",".")
            ax.XGrid = "on";
            ax.YGrid = "on";
            ax.XMinorGrid = "on";
            ax.YMinorGrid = "on";
        ...
        function generateVLine(app, event, full_x, lapA_selected_data, lapB_selected_data, lapB, NUM_VARS, axis_names, selected_vars, plot_axes)
            ...
            for i = 1:size(plot_axes,2)
                app.vlines(i) = xline(plot_axes(i), app.clickedX, "Color", vlineColor, "LineWidth", 1.5);
            end
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.fl_speed_check.Value = checkbox_states(3);
            ...
            app.fr_speed_check.Value = checkbox_states(4);
            ...
            app.rl_speed_check.Value = checkbox_states(5);
            ...
            app.rr_speed_check.Value = checkbox_states(6);
            ...
            app.vehicle_speed_check.Value = checkbox_states(7);
            ...
            app.brake_bias_check.Value = checkbox_states(8);
            ...
            app.long_gs_check.Value = checkbox_states(9);
